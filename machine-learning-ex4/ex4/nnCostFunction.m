...
J = sum(sum( (-y_matrix.*log(a3)) - (log(1-a3).*(1-y_matrix)) ))/m ;  
% Adding Regularization
T1sum = sum(sum(Theta1(:,2:end).^2)) ;
T2sum = sum(sum(Theta2(:,2:end).^2)) ;
...
%fprintf('\n Size of delta3 %f \n', size(delta3)) ;
dtemp = (delta3*Theta2) ;
delta2 = dtemp(:,2:end).*sigmoidGradient(z2) ;
%fprintf('\n size of delta2 %f \n',size(delta2)) ;
...
% Calculating grad without regularization
